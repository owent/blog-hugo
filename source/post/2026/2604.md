...
有一些其他解决方案，为了兼容老接口和流程，强行把云下的流程搬到云上（比如腾讯的tbuspp，给每个podsidecar注入一个agent进程），其实一方面有额外不必要的负担，另一方面也反而丢失了原来设计上解决的一些通道管理上的问题。
...
补充说明：这里的 *RelayHub/RelayLeaf* 用于描述在“转发拓扑/汇聚层级”上的相对位置：Hub 表示承担转发/汇聚职责的节点，Leaf 表示挂载在某个 Hub 之下的节点。实际连接与消息收发是双向的，只是我们在判定关系时会选定参考点（X）来描述相对位置。

| 关系(中文) | 新算法英文名            | 老英文名               | 定义                                                                                                                                                          |
| ---------- | ----------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 自己       | **Self**                | **self**               | $X = Y$                                                                                                                                                       |
| 代理父节点 | **ImmediateRelayHub**   | **parent**             | 若 $X \to \* $ 的消息需要经由 $Y$ 转发（$Y$ 是 $X$ 的一跳 *RelayHub*）                                                                                        |
| 间接父节点 | **TransitiveRelayHub**  | **parent**             | 若存在路径 $X \to \cdots \to Y$ 且路径上的节点均承担转发/汇聚（$Y$ 是 $X$ 的多跳 *RelayHub*）                                                                 |
| 直接子节点 | **ImmediateRelayLeaf**  | **child**              | 若 $X$ 挂载在 $Y$ 之下且 $Y$ 是 $X$ 的一跳 *RelayHub*（从 $Y$ 的视角，$X$ 是其直接 *RelayLeaf*）                                                              |
| 间接子节点 | **TransitiveRelayLeaf** | **child**              | 若 $X$ 可视作挂载在 $Y$ 之下的多跳 *RelayLeaf*（跨越若干层 Hub）                                                                                              |
| 邻居节点   | **SameHubPeer**         | 通过 **contains** 接口判定 | 若 $X$ 与 $Y$ 共享同一个 *ImmediateRelayHub*（即它们的直属 Hub 相同）                                                                                         |
| 远方节点   | **AdjacentHubPeer**     | 通过 **contains** 接口判定 | 设 $H_X$ 为 $X$ 的 *ImmediateRelayHub*，$H_Y$ 为 $Y$ 的 *ImmediateRelayHub*；若 $H_X$ 与 $H_Y$ 互为 *SameHubPeer*（Hub 层相邻），则 $X$ 与 $Y$ 互为远端对等体 |
...
- 由 **A节点 -> |代理父节点| -> B节点** , 我们可以获得直接的关系有 **Self** , **ImmediateRelayHub** , **ImmediateRelayLeaf** 。算法复杂度为O(1)。
- 如果 **A节点 -> |代理父节点| -> C节点** , **B节点 -> |代理父节点| -> C节点** 。则他们是 **SameHubPeer** 关系，算法复杂度为O(1)。
- 如果 **A节点** 的 ***代理父节点*** 链路包含 **B节点** ，或 **B节点** 的 ***代理父节点*** 链路包含 **A节点** ，它们是 **TransitiveRelayHub** 或 **TransitiveRelayLeaf** 子节点关系。这里涉及多级查找，但通常不会很深。可以考虑优化为如果深度很大，使用全局Atomic版本号比较+缓存父节点链路，如果深度小，直接链式查找。
- 其他的就全部是 **AdjacentHubPeer** 节点了。
...
**区域1-Proxy1** 和 **区域2-Proxy1** 永不互通。即便他们是 **SameHubPeer** 关系。而 **区域1-Proxy1** 和 **区域1-Proxy2** 是互通的，并且他们不需要经过 **跨大区Proxy节点** 下发直连消息就应该直接互通。
