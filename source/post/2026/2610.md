...
以后面表格里的 DeepSeek-V4-Pro / Kimi-K2.7-Code 为例，`hidden_size = 7168`，`q_lora_rank = 1536`。如果每个 head 的 Query 维度按 `qk_nope_head_dim + qk_rope_head_dim = 128 + 64 = 192` 粗算，128 个 attention head 拼起来就是：
...
$$
7168 \times 24576 \approx 176M
$$

如果拆成 `q_lora_rank = 1536` 的两段低秩投影：

$$
7168 \times 1536 + 1536 \times 24576 \approx 48.7M
$$
...
这些资源拿来干点其他更有意义的事儿它不香吗？于是就有了 **混合专家模型(MoE)** 。

**混合专家模型(MoE)** 主要是在Decoder层，把 **前馈神经网络（FNN）** 层从一个 **FNN** 变成了多个，每个都是一个专家（Expert）。
拿到输入数据的时候只会选中一个或者几个专家进行计算（即，使用子集）。
所以这里需要引入一个路由层（Router）来决定输入数据需要激活哪几个专家（Expert）？即构造这个子集。

这个Router层也通过对输入，对专家（Expert）进行打分计算概率来选取专家（Expert）。

> 最简单的策略可以是选取分数最高的一个专家（Expert），但是选取更多的专家（Expert）意味着可以激发更多的创造性。

![MoE Routing](2610-moe-routing.drawio "{max-width=800px}")

在模型加载的时候，因为所有的专家都要加载。也就是内存/显存占用是全量的会很大。但是每次计算因为只使用子集，所以会小得多。
...
**混合专家模型(MoE)** 还有个参数 moe_intermediate_size, 可以理解为：每个 MoE Expert 内部 FFN/MLP 的中间层维度。
如果模型用的是 SwiGLU 这类结构，单个 Expert 通常有三组矩阵：

- gate_proj: $d_{model}$ -> $d_{moe}$
- up_proj: $d_{model}$ -> $d_{moe}$
- down_proj: $d_{model}$ -> $d_{moe}$

所以单个 Expert 参数量粗略是：$3 \times d_{model} \times d_{moe}$ 也就是 3 \* hidden_size \* moe_intermediate_size。

一般来说，在其它条件不变、训练数据和训练预算也足够的前提下，moe_intermediate_size 变大，单个 Expert 的表达能力会更好。
但它不是越大越好，收益会递减，而且很容易被别的瓶颈卡住。比如训练和推理成本、计算量等等。

最后有一个经验是：总参数量影响世界知识，但是执行某一个领域的任务的效果和激活参数的影响更大。
