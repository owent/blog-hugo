...
async fn call_1() -> i32 {
    ...
    let ret = msg.await;
    println!("await and got {}!", ret);
    ret
}

async fn call_2() -> i32 {
    println!("ready to call call_1.");
    let res1 = call_1().await;
    let res2 = call_1().await;
    if res1 > res2 {
        println!("got first call {} greater than the second {}.", res1, res2);    
    } else {
        println!("got first call {} less than the second {}.", res1, res2);    
    }
    
    println!("call call_1 finished.");
    res1 + res2
}
...
struct TaskData {
    waker: Option<Waker>,
    future: * mut Pin<Box<dyn Future<Output = i32>>>
}
...
struct Executor {
    futures: Vec<Pin<Box<dyn Future<Output = i32>>>>
}
...
impl Executor {
    fn run(&mut self) {
        let mut allocator: i32 = 0;
        loop {
            for future in self.futures.iter_mut() {
                // 准备协程任务数据
                let mut task_data = TaskData {
                    waker: Option::None,
                    future: 0 as * mut Pin<Box<dyn Future<Output = i32>>>
                };
                let waker = unsafe{
                    Waker::from_raw(
                        RawWaker::new((&mut task_data as * mut TaskData) as *const (), &USER_WAKER_VTABLE)
                    )
                };
                task_data.waker = Some(waker);
                task_data.future = future as * mut Pin<Box<dyn Future<Output = i32>>>;
                task_data.waker.unwrap().clone().wake(); // First poll
    
                unsafe {
                    // 模拟有数据
                    if let Some(ref evt) = USER_DISPATCHER {
                        allocator = allocator + 1;
                        let _ = evt.tx.send(allocator);
...
fn main() {
    ...
    let future1 = call_1();
    let future2 = call_2();
    println!("Sizeof(call_1()): {}", size_of_val(&future1));
    println!("Sizeof(call_2()): {}", size_of_val(&future2));
    let mut exec = Executor {
        futures: vec![Box::pin(future1), Box::pin(future2)] /*转入到堆上分配*/
    };
    exec.run();
}
```
...
有兴趣的童鞋可以去 https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=4f29ba9b41d09365e6e2c2032ce06dec 这里自己看运行结果。
