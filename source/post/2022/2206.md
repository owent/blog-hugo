...
`return_value()/return_void()` 不能 `handle.destroy()` 。否则GCC的实现中会重复析构协程栈变量。
...
实现一个不带atomic非线程安全的 `shared_ptr` ，很多线程内部流程需要引用计数，但是其实不需要atomic操作。这会导致额外的CPU Cache失效。(特别是 `task_future` 和 `generator_future` 里 awaitable和promise对象之间交互时对 `context` 的传递，一定是在单线程内的。 )
