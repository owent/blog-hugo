...
tags: 
  ...
  - Attention
  - MoE
title: Transformer 入坑笔记（一） - 大模型训练链路
type: post
---
...
这张表只看量级，别拿它当训练配置。实际成本还会受到参数规模、MoE 激活参数量、上下文长度、batch size、并行策略、显卡或 NPU 型号、数据质量和训练目标影响。
...
这里也别把话说得太绝对：学生模型在某个窄任务上有机会超过教师模型，尤其是数据清洗、任务定义和评估集都做得更贴近目标场景的时候。但蒸馏不能凭空造出教师模型自己都不稳定的能力，更不能替代目标场景下的评估。

另外一个很现实的问题是，蒸馏数据覆盖不到的长尾场景，小模型往往掉得更快。平均分可能还能看，但一到边角 case 就开始露馅。做蒸馏时，评估不能只看通用 benchmark，还要看目标场景下是否真的更省钱、更快、并且效果仍然可接受。

## 继续深入

这些代码容易让流程显得很简单：读数据、tokenize、train、save。但真正难的不是把 `Trainer` 跑起来，而是数据怎么清、任务怎么定义、评估怎么做、长尾 case 怎么兜住，以及训练出来以后能不能稳定、便宜、可控地部署。

下一篇我准备顺着 Transformer 往下看：token、embedding、attention、MoE 这些东西到底在模型里各自负责什么。
