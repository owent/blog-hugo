...
不过单单就[Hexo][1]和[Hugo][2]的框架设计来说，我觉得是[Hexo][1]远远胜于[Hugo][2]的。因为[Hexo][1]有更完整的插件机制，主题机制。它的插件机制可以让你替换里面的核心组件，增加更多的渲染引擎、模板引擎等等。并且和nodejs的很多工具配合起来用，非常方便。这样主题作者和插件作者都能按自己的习惯和熟悉的东西来。比如你可以用sass或者less来写css，然后也可以不用默认的模板引擎ejs，很多人的主题用jade的。甚至可以换掉Markdown引擎，增加更多的Markdown功能进去。也可以在很多事件点插入和修改生成的HTML，来插入一些功能。
...
现在我也说不上来这两个那个更有前景，[Hexo][1]还差很多文档和事件响应目前给了裸HTML，操作起来其实很不方便。而[Hugo][2]目前确实插件机制，而且很多配置和参数写得很死。我迁移到静态博客系统主要是复刻主题。所以两个系统的主题都复刻了一遍，主框架是一样的，但是用了兼容两者的写法和配置吧。
...
然后在原有的主题上有做了更进一步地精简，裁剪加载文件大小和把一些突变换成纯CSS3实现，再就是微调一些样式。
...
这个在[Hexo][1]里很方便，有人写了插件，安装就行了。但是[Hugo][2]就得我自己集成了，还好反正不困难。然后我的Blog就支持公式了。并且我把以前一些文章的内容改成了公式。好看多了。
...
不过我之前看到过有人做了用Github的Issue来做评论系统的，之前star了[gitcomment][4]，就试了一下。我的主题里是集成了这个，但是这个和另一个俄罗斯小朋友写得系统一样，要求评论者必须授权Application对Public仓库的完整读写权限（包括但不限于代码、Wiki和issues）。不过[gitcomment][4]作者也说了，github没有提供更系粒度的权限了，所以这已经是要的最小的权限了,但是读权限就算了，还必须给写权限，我还是觉得有点不靠谱。

所以最终还是回归[disqus][5]。无奈这货在国内被墙。唉，反正上我blog还会回复的应该都是程序员了吧？程序员翻墙不是难事吧？所以不管了先还是[disqus][5]吧。
