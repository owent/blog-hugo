...
- 默认文风应自然、圆润、连贯，像成熟工程师写给同行看的技术博客；避免提纲硬展开、翻译腔、宣传腔和明显的 AI 拼接感。
...
- 少用提示词式口号，例如"更稳"、"不猜测"、"更安全"、"更像助手"、"守住边界"、"收敛输出"。需要表达这类意思时，改成具体行为、约束或验证方法，例如"拒答边界"、"偏好数据"、"评估集覆盖长尾 case"。
- 避免连续使用"不是 A，而是 B"、"不只是 A，还要 B"、"从 X 到 Y"这类对偶句。它们偶尔可用，但一段里反复出现会很像 AI 总结。
- 样例代码、粗略数字、假设路径只需要短边界说明。优先写"下面四段代码只是帮助理解训练骨架"这类一句话；不要连续解释"不是生产脚本、不能直接跑、真实场景还要补很多工程细节"。
- 比喻要克制。不要为了降低门槛频繁使用"大脑"、"拷出来"、"像一个可上线助手"这类比喻；技术文章优先用准确名词和具体场景。
- 避免用"下面这些数字只用来建立量级感，不对应任何官方模型的完整配置"这类长免责声明。可以改成"这张表只看量级，别拿它当训练配置"。
- 润色时先扫描上述句式和口头禅，再改结构和语气；不要只替换用户点名的单句。
...
- 润色或扩写时，优先把半句口语、记录式笔记、生硬直译和残缺表达改成自然连贯的博文表述，但不要把全文修成失去作者判断和语气的“统一 AI 腔”
- 扩写文章时，优先补"为什么这样做""什么时候适用""什么时候不适用""怎么验证""常见坑"；必要时补一张真正减少理解成本的图，而不是只为了篇幅而加水
- 涉及仓库里已有的相近主题时，先搜索相关文章，避免重复铺陈同一套背景，必要时通过链接建立上下文
- 如果用户指出某类 AI 腔或过度解释，必须先总结文章里的同类模式，再统一处理；不要只改用户点名的一两句话。
...
- [ ] 术语一致，前后统一
- [ ] 文风自然圆润，没有明显的提纲腔、翻译腔或 AI 拼接感
- [ ] 没有密集出现"不是 A 而是 B"、"不只是 A"、"更稳/更安全/更像"这类 AI 常用句式
- [ ] 样例代码、粗略数字和假设路径的边界说明简短，不反复免责声明
