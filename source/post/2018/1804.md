...
泛型和操作符重载
-----------------------
[Rust][1]的泛型没什么太多可说的。大体上就是 ```<T: Display + Clone>``` 或```<T> where T: Display + Clone``` 这两种形式。

[Rust][1]的操作服重载相当于是要实现 **std** 库里 **PartialOrd** 的 **traits** 。不过 **std** 库是可以禁用掉的，这种情况似乎操作符重载也GG了。
