...
第二个坑，是临时对象的析构顺序， `future` 对象析构和 `final_suspend` 的执行顺序未定义。在GCC（至少是某些版本的GCC）中，如果我们执行 `co_await a{...};` ，然后 `a::operator await` 返回 `b` 类型。都是临时变量，那么 `a` 可能在 `b` 创建后，调用 `b::await_suspend` 之前析构（不同编译器行为不完全一样）。这里如果有资源引用可能主要注意一下生命周期。
