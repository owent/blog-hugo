...
1. 针对 OLTP 操作，使用中性化策略，Client直接访问主节点；针对 OLAP 操作，采用分布式策略，Client执行仅使用快照读。
2. 依据 **Spanner**[^spanner] ，整体是流式地执行和返回，数据按Hash分布。
    > 因为所有数据都是远程的，所以不能像传统数据库那样把相近的放一起，也没办法使用本地数据优化Batch读取。不过有个好处是不用再关心底层存储和延迟，二而且按Hash地流式地流程很容易控制内存使用量。
