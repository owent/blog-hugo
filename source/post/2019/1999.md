...
[Anna][^anna] 的key和节点的分布也是一致性哈希。采用 [gossip][^gossip] 算法来处理actor的容灾和扩缩容。按文中的意思似乎数据分片的同步也采用的是 [gossip][^gossip] 。不过在这一点上 [gossip][^gossip] 对分布式系统的局部分片扩容是比 [Paxos][17] 和 [Raft][18] 要好很多。

> 我的理解上还是觉得每个数据分片需要有主处理节点来处理写的，否则冲突率会比较高。后面文中也提到了在某些一致性策略下冲突率提高了也会影响数据同步的开销和扩容效果。
...
如图，在低争用的情况下，[Anna][^anna] 的表现和 [redis-cluster][^redis] 相近。在高争用的时候，

另外它也说明了，如果一致性策略选的比较弱的话，高争用的情况下还会有Merge开销突增的情况。
