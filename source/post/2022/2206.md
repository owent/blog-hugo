...
第二个坑，是临时对象的析构顺序，如果 `co_await` 一个 `future` 对象，返回 `awaitable` 用于协程切出和恢复，在GCC 10的某些版本中，这个 `future` 对象析构会在 `await_suspend` 的之前完成（ 10.3 以后的版本）。这里如果有资源引用可能主要注意一下生命周期。具体可以看一下 https://godbolt.org/z/dbrxns66b 这里的sample。
